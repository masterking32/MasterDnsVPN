...
| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `UDP_HOST` | `"::"` | IP بدون براکت؛ اگر خالی باشد همین مقدار استفاده می‌شود | آدرسی که سرور DNS روی آن bind می‌شود.<br>wildcard پیش‌فرض IPv6 (`::`) در سیستم‌های سازگار، ترافیک IPv4 نگاشت‌شده را نیز می‌پذیرد تا یک socket هم IPv6 و هم IPv4 را دریافت کند؛ در غیر این صورت فقط IPv6 باقی می‌ماند. برای wildcard مربوط به IPv4 از `0.0.0.0` استفاده کنید. یک آدرس مشخص، listener را به همان خانواده محدود می‌کند. |
