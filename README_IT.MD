...
| Parametro | Valore di Esempio | Valori Consentiti / Comportamento Reale | Spiegazione Completa |
| :--- | :--- | :--- | :--- |
| `UDP_HOST` | `"::"` | indirizzo IP letterale senza parentesi; se vuoto, viene usato questo valore | Indirizzo su cui il server DNS si lega.<br>Il wildcard IPv6 predefinito (`::`) accetta il traffico IPv4 mappato dove supportato, quindi un solo socket riceve sia IPv6 sia IPv4; altrimenti rimane solo IPv6. Usa `0.0.0.0` per il wildcard IPv4. Un indirizzo concreto limita il listener a quella famiglia. |
