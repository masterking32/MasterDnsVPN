...
| :--- | :--- | :--- | :--- |
| `UDP_HOST` | `"0.0.0.0"` | если поле пустое, используется значение по умолчанию. | Адрес, на котором привязан DNS-сервер. <br>Универсальный UDP-адрес (`0.0.0.0` или `::`) принимает IPv4 и IPv6 в системах с поддержкой IPv4-mapped IPv6; в остальных случаях Go использует доступное семейство адресов. Конкретный адрес ограничивает listener этим семейством. |
