...
| :--- | :--- | :--- | :--- |
| `UDP_HOST` | `"::"` | IP-литерал без скобок; если поле пустое, используется значение по умолчанию. | Адрес, на котором привязан DNS-сервер. <br>IPv6-адрес по умолчанию (`::`) принимает IPv4-mapped трафик там, где это поддерживается, поэтому один сокет обслуживает IPv6 и IPv4; в противном случае он остается только IPv6. Используйте `0.0.0.0` для IPv4 wildcard. Конкретный адрес ограничивает listener этим семейством. |
