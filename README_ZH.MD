...
| 参数 | 示例值 | 允许的值 / 实际行为 | 完整说明 |
| :--- | :--- | :--- | :--- |
| `UDP_HOST` | `"::"` | 不带方括号的 IP 字面量；若为空则使用此值 | DNS 服务器绑定的地址。<br>默认 IPv6 通配地址（`::`）在系统支持时也接受 IPv4 映射流量，因此一个套接字可同时接收 IPv6 和 IPv4；否则它仅使用 IPv6。使用 `0.0.0.0` 可请求 IPv4 通配地址。指定具体地址会将监听器限制为该地址族。 |
